...
private const DESTRUCTIVE = ['destroyCourse', 'destroyAllStudentsFromCourse', 'deleteResource', 'deleteActivities'];
    ...
    public function handle(Request $request): JsonResponse
    {
        ...
        // <<< CAMBIO: Detectar si quiere borrar un mes completo
        if ($hasDeleteIntent && empty($intentDeleteDates)) {
            $monthRange = $this->parseFullMonthRange($prompt ?: $rawMessage);
            if ($monthRange) {
                $intentDeleteDates = $monthRange;
            }
        }
        ...
        $response = Http::timeout(120)
            ...
            ->post('https://api.openai.com/v1/chat/completions', [
                'model'       => 'gpt-4.1', // Mantengo el solicitado
                'temperature' => 0,
                'tools'       => $this->toolDefinitions(),
                'messages'    => [
                    ['role' => 'system', 'content' => "Eres Nova, motor central académico con permisos totales de creación y gestión pedagógica. Tu prioridad es ejecutar lo que el docente pide (crear clases, tareas, evaluaciones, adaptaciones NEE y planificaciones) con autonomía y criterio pedagógico. REGLA CRÍTICA: SOLO puedes usar herramientas destructivas (deleteResource/deleteActivities) si el usuario pide explícitamente borrar/eliminar/limpiar/vaciar. Si el usuario pide planificar/crear/generar/organizar, está prohibido usar borrado. Si faltan datos, asume valores coherentes y reporta lo asumido. Fecha actual: $today. Contexto de cursos del profesor:\n$coursesContext\nContexto vivo actual:\n$contextJson"],
        ...
        // <<< CAMBIO COMPLETO: Nueva lógica de filtrado de herramientas destructivas
        if ($hasDeleteIntent) {
            // Si hay intención de borrado, FILTRAR para quedarnos SOLO con acciones destructivas
            $toolCalls = array_values(array_filter($toolCalls, function ($tc) {
                $fn = $tc['function']['name'] ?? '';
                return in_array($fn, self::DESTRUCTIVE, true);
            }));
            
            // Si no hay toolCalls destructivas pero el usuario quiere borrar, 
            // intentar borrado por fechas directamente
            if (empty($toolCalls)) {
                $intentDeleteDates = $this->parseDatesFromText($prompt ?: $rawMessage);
                $intentCourseId = $this->detectCourseContext($prompt ?: $rawMessage, $teacher);
                
                if (!empty($intentDeleteDates) && $intentCourseId) {
                    $result = $this->doDeleteActivities([
                        'course_id' => $intentCourseId,
                        'start_date' => $intentDeleteDates[0]->format('Y-m-d'),
                        'end_date' => end($intentDeleteDates)->format('Y-m-d'),
                    ], $teacher->id);
                    return response()->json($result);
            ...
            // Si no hay intención de borrado, eliminar herramientas destructivas
            $toolCalls = array_values(array_filter($toolCalls, function ($tc) {
                $fn = $tc['function']['name'] ?? '';
                return ! in_array($fn, self::DESTRUCTIVE, true);
            }));
        ...
        if (empty($toolCalls)) {
            return response()->json(['message' => $message['content'] ?? 'No se detectaron acciones.']);
        }
        ...
        if ($destructiveFound->isNotEmpty() && !$confirmed) {
            return response()->json(['requires_confirmation' => true, 'actions' => $toolCalls]);
        }
    ...
    // <<< NUEVA FUNCIÓN: Para detectar meses completos como "todo marzo"
    private function parseFullMonthRange(string $text): ?array
    {
        $meses = [
            'enero' => 1, 'febrero' => 2, 'marzo' => 3,
            'abril' => 4, 'mayo' => 5, 'junio' => 6,
        ...
        // Buscar "todo [el mes]" o "todo marzo"
        if (preg_match('/todo\s*(el)?\s*(' . implode('|', array_keys($meses)) . ')/iu', $text, $matches)) {
            $monthName = mb_strtolower($matches[2]);
            $month = $meses[$monthName] ?? null;
            
            if ($month) {
                $year = now()->year;
                // Si menciona un año específico
                if (preg_match('/\b(20\d{2})\b/', $text, $yearMatch)) {
                    $year = (int) $yearMatch[1];
                }
    ...
    private function buildNeeAdaptation(string $neeType): string
    {
        return match (mb_strtolower($neeType)) {
            ...
            'tea', 'autismo', 'tea/autismo', 'tea - autismo' => "Para este alumno con TEA, anticipa la secuencia con un mini-guion visual, reduce estímulos distractores y ofrece ejemplos concretos. Permite tiempos de respuesta más ampl